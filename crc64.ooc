CRC64: class {
    ...
    ISO: const UInt64 = 0xD800000000000000
    ECMA: const UInt64  = 0xC96C5795D7870F42

    init: func{ hash = 0 }

    reset: func{ hash = 0 }

    makeTable: func(seed: UInt64){
        for(i in 0..0x100){
            crc := i as UInt64
            for(j in 0..8){
                if(crc & 1){ crc = (crc >> 1) ^ seed }
                else { crc = crc >> 1 }
            }
            crc64Table[i] = crc
        }
    ...
    write: func(crcdata: UInt8*, length: UInt64){
        hash = ~hash
        for(i in 0..length){
            hash = crc64Table[((hash ^ crcdata[i]) & 0xFF)] ^ (hash >> 8)
        }
        hash = ~hash
    }

    checksum: func -> UInt64{ hash }
}
